    ...
    multipleTouchProbability: Double = 0.05) {
        addAction(weight: weight) { [weak self] in
            ...
            if #available(iOS 9.0, *) {
                let coordinate = self?.currentApp.coordinate(withNormalizedOffset: CGVector(dx: locations[0].x/((self?.currentApp.frame.maxX)!/2), dy: locations[0].y/((self?.currentApp.frame.maxY)!/2)))
                coordinate!.tap()
