...
public class Monkey {
    ...
    */
    public convenience init(frame: CGRect, application:XCUIApplication) {
        let time = Date().timeIntervalSinceReferenceDate
        let seed = UInt32(UInt64(time * 1000) & 0xffffffff)
        self.init(seed: seed, frame: frame, application:application)
    }
    ...
    /**
        ...
        - parameter frame: The frame to generate events in.
          Should be set to the size of the device being tested.
    */
    public init(seed: UInt32, frame: CGRect, application:XCUIApplication) {
...
//                }
                ...
                if self.currentApp.buttons["密码登录"].exists{
                    self.login(app: self.currentApp, user: "15154111005", password: "654321")
                }
                action()
            }
